...
class DAC(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        wiring.connect(m, wiring.flipped(self.inputs[0]), self.fifo_0.w_stream)
        wiring.connect(m, wiring.flipped(self.inputs[1]), self.fifo_1.w_stream)
        m.d.comb += [
            self.fifo_0.r_en.eq(self.latch & self.fifo_0.r_rdy),
            sample_0.eq(self.fifo_0.r_data),
            self.fifo_1.r_en.eq(self.latch & self.fifo_1.r_rdy),
            sample_1.eq(self.fifo_1.r_data),
        ]
